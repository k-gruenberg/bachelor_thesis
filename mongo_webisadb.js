...
// (7) Here is a version that also considers pre- and post-modifier:

let searchTermPreMod = "darth";
let searchTerm = "vader";
let searchTermPostMod = "";
let topK = 25;
let res = []; // sorted results
let collectionsDoneCounter = 0;
db.getCollectionNames().forEach(function(collectionName) {
    db[collectionName].find({instance: searchTerm}).forEach(function(item) {
        item.modifications.forEach(function(m) {
            let indexToInsert = 0;
            while (indexToInsert < res.length && res[indexToInsert][6] >= m.frequency) {indexToInsert++;}
            if ((m.ipremod == searchTermPreMod) && (m.ipostmod == searchTermPostMod)) {
                res.splice(indexToInsert, 0, [m.ipremod, item.instance, m.ipostmod, m.cpremod, item.class, m.cpostmod, m.frequency]);
            }
        });
    });
...
// darth | vader |  |  | none |  | 41
// darth | vader |  |  | dad |  | 34
// darth | vader |  | iconic | character |  | 34
// darth | vader |  | iconic | character |  | 34
// darth | vader |  |  | dad |  | 34
// darth | vader |  |  | dad | like any otherexcept | 29
// darth | vader |  |  | dad | like any otherexcept | 29
// darth | vader |  | good | father |  | 21
// darth | vader |  | villains playable | character |  | 21
// darth | vader |  |  | great |  | 21
// darth | vader |  | villains playable | character |  | 21
// darth | vader |  | good | father |  | 21
// darth | vader |  |  | great |  | 21
// darth | vader |  | bad | guy |  | 20
// darth | vader |  | bad | guy |  | 20
// darth | vader |  | unwanted | change |  | 18
// darth | vader |  | unwanted | change |  | 18
// darth | vader |  | other star wars | character |  | 17
